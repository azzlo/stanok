class ServiceChargesController < ApplicationController
  ...
  def new
    ...
    @service_charge = @service.service_charges.build
  end
  ...
  def create
    @service_charge = @service.service_charges.build(service_charge_params)

    respond_to do |format|
      if @service_charge.save
        format.html { redirect_to @service, notice: 'Service charge was successfully created.' }
  ...
  private
    ...
    def service_charge_params
      params.require(:service_charge).permit(:amount, :description, :service_id)
    end
end
